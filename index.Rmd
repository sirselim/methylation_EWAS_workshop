...
The Q-value is an adjustment for multiple correction bias. We're going to extract all CpG sites which have a Q-value less than 0.0001.

First, how many sites will this be?

```{r, message=FALSE, warning=FALSE}
# explore some results
table(results$qval < 0.0001)
```

Now extract these sites into an object called `top.results`:

```{r, message=FALSE, warning=FALSE}
top.results <- results[results$qval <= 0.0001,]
```
